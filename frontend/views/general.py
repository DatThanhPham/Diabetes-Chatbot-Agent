...
def show_visualize(user: dict | None = None):
    """
    Trang phân tích dữ liệu tiểu đường (như một page giống Dashboard).
    """

    st.title("☁️ DJAT CLOUD: TRÍ TUỆ NHÂN TẠO VỀ SỨC KHỎE BỆNH TIỂU ĐƯỜNG")

    # 1. Chọn file dữ liệu (giữ nguyên logic session_state)
    ...
    # 🔹 Thay vì st.sidebar.radio, đặt mode ngay trên main content
    ...
    if mode == "Tổng quan về dữ liệu":
        ...
        # 6. Biểu đồ đường Diabetes theo độ tuổi & giới
        ...
        # 7. Biểu đồ mối quan hệ Diabetes vs feature khác + giới
